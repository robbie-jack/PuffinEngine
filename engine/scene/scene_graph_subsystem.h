...
namespace puffin
{
	...
	namespace scene
	{
		...
		private:
			...
			T* AddNodeInternal(const std::string& name, UUID id = gInvalidID, UUID parent_id = gInvalidID)
			{
				...
				if (id == gInvalidID)
				{
					node = GetPool<T>()->AddNode(m_engine, name);
					id = node->GetID();
				}
				else
				{
					node = GetPool<T>()->AddNode(m_engine, name, id);
				}
			...
			Node* AddNodeInternal(uint32_t typeID, const std::string& name, UUID id = gInvalidID, UUID parentID = gInvalidID)
			{
				...
				if (id == gInvalidID)
				{
					node = GetPool(typeID)->AddNode(m_engine, name);

					id = node->GetID();
				}
				else
				{
					node = GetPool(typeID)->AddNode(m_engine, name, id);
				}
	...
	{
		template<>
		inline std::string_view GetTypeString<scene::SceneGraphSubsystem>()
		{
			return "SceneGraphSubsystem";
		}

		template<>
		inline entt::hs GetTypeHashedString<scene::SceneGraphSubsystem>()
		{
			return entt::hs(GetTypeString<scene::SceneGraphSubsystem>().data());
		}
