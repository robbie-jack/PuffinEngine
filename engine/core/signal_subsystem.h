...
namespace puffin
{
	namespace core
	{
		...
		class SignalSubsystem : public EngineSubsystem
		{
		public:

			explicit SignalSubsystem(const std::shared_ptr<core::Engine>& engine) : EngineSubsystem(engine)
			{
			}
	...
	{
		template<>
		inline std::string_view GetTypeString<core::SignalSubsystem>()
		{
			return "SignalSubsystem";
		}

		template<>
		inline entt::hs GetTypeHashedString<core::SignalSubsystem>()
		{
			return entt::hs(GetTypeString<core::SignalSubsystem>().data());
		}
