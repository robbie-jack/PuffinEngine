...
namespace Puffin::Physics
{
	void Box2DContactListener::BeginContact(b2Contact* contact)
	{
		...
		collisionBeginEvent.entityA = static_cast<ECS::Entity>(contact->GetFixtureA()->GetBody()->GetUserData().pointer);
		collisionBeginEvent.entityB = static_cast<ECS::Entity>(contact->GetFixtureB()->GetBody()->GetUserData().pointer);
	...
	void Box2DContactListener::EndContact(b2Contact* contact)
	{
		...
		collisionEndEvent.entityA = static_cast<ECS::Entity>(contact->GetFixtureA()->GetBody()->GetUserData().pointer);
		collisionEndEvent.entityB = static_cast<ECS::Entity>(contact->GetFixtureB()->GetBody()->GetUserData().pointer);
